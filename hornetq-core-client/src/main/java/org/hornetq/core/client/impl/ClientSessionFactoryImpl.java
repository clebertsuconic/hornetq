...
public class ClientSessionFactoryImpl implements ClientSessionFactoryInternal, ConnectionLifeCycleListener
{
  ...
  // Constants
   ...
   private void failoverOrReconnect(final Object connectionID, final HornetQException me)
   {
      ...
      {
         if (connection == null || connection.getID() != connectionID || !clientProtocolManager.isAlive())
         {
         ...
         if (reconnectAttempts != 0)
         {
            ...
            if (lock != null)
            {
               ...
               clientProtocolManager.cleanupBeforeFailover();
                                                 ...
                                                 final boolean preAcknowledge) throws HornetQException
   {
      synchronized (createSessionLock)
      {
         return clientProtocolManager.createSessionChannel(name, username,
   ...
   protected Lock lockSessionCreation()
   {
      synchronized (failoverLock)
      {
         return clientProtocolManager.lockSessionCreation();
      }
   }
